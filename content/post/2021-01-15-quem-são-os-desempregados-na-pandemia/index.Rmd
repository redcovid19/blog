...
pnad_covid <- read_csv("data/PNAD_COVID_102020.csv", col_types = cols(.default = "d"))

# Criando colunas com Variáveis 
pnad <- pnad_covid %>% mutate(one = 1,
...
#Com peso amostral 
pnad_com_pesos_mapa <- pnad %>% as_survey_design(ids = UPA, strata = Estrato, weights = V1032, nest = TRUE)
pnad_com_pesos = pnad_com_pesos_mapa  %>% filter(CAPITAL == "33")
...
desocup_mapa =read_excel("data/taxa_desocup_outubro.xlsx")
desocup_mapa$Taxa_desocup = round(desocup_mapa$Taxa_desocup, 1)

mapa = geojson_read("data/br_states.geojson", what = "sp")

desocup_mapa$codigo_ibg= as.character(desocup_mapa$codigo_ibg)
